...
telegram_api = Bot(token=os.getenv("TELEGRAM_BOT_TOKEN"))

def send_telegram_message(chat_id, text):
    try:
        # Se agrega el parámetro timeout para evitar tiempos de espera demasiado cortos.
        response = telegram_api.send_message(chat_id=chat_id, text=text, timeout=20)
    except Exception as e:
        _fallback_logger.error("Error enviando mensaje: %s", e)
        try:
            response = telegram_api.send_message(chat_id=chat_id, text=text, timeout=20)
        except Exception as retry_e:
            _fallback_logger.error("Reintento fallido al enviar mensaje: %s", retry_e)
            raise
